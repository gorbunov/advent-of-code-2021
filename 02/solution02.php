...
use function Gorbunov\functional\collection\slice;
...
$solver01 = compose(
    reducer([0, 0, 0],
        fn($carry, $command) => match ($command[0]) {
...
$solver02 = compose(
    reducer(
        [0, 0, 0],
        fn($carry, $command) => match ($command[0]) {
            CommandList::FORWARD => [$carry[0] + $command[1], $carry[1] + $command[1] * $carry[2], $carry[2]],
            CommandList::DOWN => [$carry[0], $carry[1], $carry[2] + $command[1]],
            CommandList::UP => [$carry[0], $carry[1], $carry[2] - $command[1]],
        },
    ),
    slice(0, 2),
    array_product(...),
